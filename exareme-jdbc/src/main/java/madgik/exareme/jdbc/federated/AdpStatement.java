...
public class AdpStatement implements Statement {
    ...
    public AdpStatement(AdpConnection connection, int resultSetType, int resultSetConcurrency) {
        this.con = connection;
        this.closeOnCompletion = false;
        this.resultSet = null;
        this.g = new Gson();
    ...
    @Override public AdpResultSet executeQuery(String sql) throws SQLException {

        if (sql.startsWith("addFederatedEndpoint") || sql.startsWith("removeFederatedEndpoint")) {
            AdpRequest q = new AdpRequest(sql, "query", con.getDbPath());
            getResponse(q);
            ...
            for (Schema endpoint : this.con.getFederatedConnections().getSchemas()) {
                if (table.startsWith(endpoint.getId() + "_")) {
                    ...
                    try (Statement s = remote.createStatement()) {
                        first = s.executeQuery(
                            "select * from " + endpoint.getSchema() + "." + remoteTableName
                                + " where 0=1;");
                    ...
                    first.close();
                    this.resultSet = new AdpResultSet(
                        new InputStreamReader(new StringBufferInputStream(sb.toString())),
    ...
    @Override public int executeUpdate(String sql) throws SQLException {
        AdpRequest q = new AdpRequest(sql, "update", con.getDbPath());
        getResponse(q);
    ...
    @Override public int getQueryTimeout() throws SQLException {
        return httpclient.getParams().getIntParameter(CoreConnectionPNames.SO_TIMEOUT, 0) / 1000;
    ...
    @Override public void setQueryTimeout(int seconds) throws SQLException {
        ...
        httpclient.getParams().setParameter(CoreConnectionPNames.SO_TIMEOUT, seconds * 1000);
    }
    ...
    @Override public boolean execute(String sql) throws SQLException {
        AdpRequest q = new AdpRequest(sql, "execute", con.getDbPath());
        getResponse(q);
        BufferedReader br = new BufferedReader(result);
        String line;
        try {
