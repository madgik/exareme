...
from ( select rid, clid, sum( (val-clval) * (val-clval) ) as dist
       from ( select rid, colname, val, clid, clval
              from ( select * from inputlocaltbl where colname in ('%{column1}', '%{column2}') )
              join clustercenters
...
select clid,
       ...
       avg(val) as clval
from ( select * from inputlocaltbl where colname in ('%{column1}', '%{column2}') ) as h,
