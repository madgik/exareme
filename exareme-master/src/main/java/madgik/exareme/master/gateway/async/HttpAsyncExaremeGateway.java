...
public class HttpAsyncExaremeGateway implements ExaremeGateway {
    ...
    public HttpAsyncExaremeGateway(AdpDBManager manager) throws Exception {
        ...
        final IOReactorConfig reactorConfig =
            IOReactorConfig.custom().setIoThreadCount(1).setSoKeepAlive(true)
        ...
        final ConnectionConfig connectionConfig =
            ConnectionConfig.custom().setBufferSize(ExaremeGatewayUtils.GW_BUFFERSIZE_KB)
                .setFragmentSizeHint(ExaremeGatewayUtils.GW_BUFFERSIZE_KB).build();

        final HttpProcessor httpproc =
            new ImmutableHttpProcessor(new ResponseDate(), new ResponseServer("ExaremeGateway/0.1"),
        ...
        registry
            .register(ExaremeGatewayUtils.GW_API_EXPLAINQUERY, new HttpAsyncExplainQueryHandler());
        registry
            .register(ExaremeGatewayUtils.GW_API_STREAMQUERY, new HttpAsyncStreamQueryHandler());
        registry.register(ExaremeGatewayUtils.GW_API_STREAMQUERY_RESULT,
            new HttpAsyncResultStreamQueryHandler());
        registry.register(ExaremeGatewayUtils.GW_API_STREAMQUERY_DELETE,
            new HttpAsyncDeleteStreamQueryHandler());
        registry.register(ExaremeGatewayUtils.GW_API_STREAMQUERY_INFO,
            new HttpAsyncStreamQueryInfoHandler());
        registry.register(ExaremeGatewayUtils.GW_API_HISTORICALSTREAMQUERY,
            new HttpAsyncOptiqueHistoricalQueriesHandler());


        Class.forName(
            "madgik.exareme.master.gateway.OptiqueStreamQueryMetadata.StreamRegisterQuery");

        final HttpAsyncService handler =
            new HttpAsyncService(httpproc, null, null, registry, null, null);
