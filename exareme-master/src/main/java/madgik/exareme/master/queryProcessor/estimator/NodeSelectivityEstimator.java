...
public class NodeSelectivityEstimator implements SelectivityEstimator {
    ...
    private static final org.apache.log4j.Logger log =
        org.apache.log4j.Logger.getLogger(SelectivityEstimator.class);
    ...
    public NodeSelectivityEstimator(String json) throws Exception {
        ...
        //        try {
        //            keys = di.madgik.decomposer.util.Util.getMysqlIndices("jdbc:mysql://10.240.0.10:3306/npd?"
        //                    + "user=benchmark&password=gray769watt724!@#");
        //        } catch (SQLException ex) {
        //            Logger.getLogger(NodeSelectivityEstimator.class.getName()).log(Level.SEVERE, null, ex);
        //        }
        ...
        //            else{
        //                Logger.getLogger(NodeSelectivityEstimator.class.getName()).log(Level.WARNING, "Table {0} does not exist in stat json file", table);
        //            }
        //        }
    ...
    @Override public void makeEstimationForNode(Node n) {
        try {
            ...
            log.error("cannot compute selectivity for node " + n.getObject().toString() + ":" + ex
                .getMessage());
        }
    ...
    public void estimateFilter(Node n, Selection s, Node child) {
        ...
        for (Operand nextFilter : filters) {
            if (nextFilter instanceof UnaryWhereCondition) {
                //normally you don't care for these conditions (Column IS NOT NULL)
                ...
                //                else f = new Filter(col.tableAlias, col.columnName, FilterOperand.NotEqual, Double.parseDouble(con.toString()));

                //adjust RelInfo's histograms based on the resulting histogram
                resultRel.adjustRelation(col.columnName, resultHistogram);
    ...
    public void estimateJoin(Node n, NonUnaryWhereCondition nuwc, Node left, Node right) {
        ...
        Histogram resultHistogram = resultRel.getAttrIndex().get(l.columnName).getHistogram();

        resultHistogram.join(rRel.getAttrIndex().get(r.columnName).getHistogram());
        ...
        //adjust RelInfo's histograms based on the resulting histogram
        resultRel.adjustRelation(l.columnName, resultHistogram);

        //fix alias mappings to RelInfo. The joining aliases must point to the same RelInfo after the join operation        
        schema.getTableIndex().put(l.tableAlias, resultRel);
        schema.getTableIndex().put(r.tableAlias, resultRel);
