...
public class Util {
    ...
    public static HashMap<String, HashSet<String>> getMysqlIndices(String conString)
        throws SQLException {
    ...
    public static Set<String> getAnalyzeColumns(List<String> queries) {
        ...
        for (String q : queries) {
            ...
            for (Column c : query.getAllReferencedColumns()) {
                if (!tables.containsKey(c.tableAlias)) {
                    tables.put(c.tableAlias, new HashSet<String>());
                }
                Set<String> cols = tables.get(c.tableAlias);
