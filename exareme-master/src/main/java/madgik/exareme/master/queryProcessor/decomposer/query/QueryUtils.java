    ...
    public static Operand getOperandFromNode(ValueNode node) {
        ...
        if (node instanceof ColumnReference) {
            ...
            if (jvn instanceof StaticMethodCallNode) {
                StaticMethodCallNode call =
                    (StaticMethodCallNode) ((JavaToSQLValueNode) node).getJavaValueNode();
                ...
                try {
                    NonStaticMethodCallNode call =
                        (NonStaticMethodCallNode) ((JavaToSQLValueNode) node).getJavaValueNode();
            ...
            if (cNode.getType().getSQLstring().equalsIgnoreCase("char(1)")) {
                CastOperand co =
                    new CastOperand(QueryUtils.getOperandFromNode(cNode.getCastOperand()),
                        "CHAR(8000)");
                result = co;
            } else {
                CastOperand co =
    ...
    public static Operand convertToMySQLDialect(Operand o) {
        ...
        if (o instanceof CastOperand) {
            ...
            if (co.getCastType().equalsIgnoreCase("INTEGER")) {
                CastOperand signed =
                    new CastOperand(convertToMySQLDialect(co.getCastOp()), "SIGNED");
                return signed;
            }
            if (co.getCastType().equalsIgnoreCase("DOUBLE") || co.getCastType()
                .equalsIgnoreCase("FLOAT")) {
                CastOperand signed =
                    new CastOperand(convertToMySQLDialect(co.getCastOp()), "DECIMAL");
                return signed;
            }
            ...
            if (co.getCastType().equalsIgnoreCase("TIMESTAMP")) {
                CastOperand signed =
                    new CastOperand(convertToMySQLDialect(co.getCastOp()), "DATETIME");
                return signed;
            }
            return co;
        }
