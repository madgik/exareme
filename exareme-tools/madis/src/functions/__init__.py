            ...
            if settings['logging']:
                try:
                    ...
                    if hasattr(lg.logger.parent.handlers[0], 'baseFilename'):
                        lg.info("%s(%s)" % (func.__name__, ','.join(
                            list([repr(el) for el in args[1:]]) + ["%s=%s" % (k, repr(v)) for k, v in kw.items()])))
                except Exception:
                    pass
            print "%s(%s)" % (func.__name__, ','.join(
                list([repr(el)[:200] + ('' if len(repr(el)) <= 200 else '...') for el in args[1:]]) + [
                    "%s=%s" % (k, repr(v)) for k, v in kw.items()]))
        return func(*args, **kw)
...
class Cursor(object):
    ...
    def __setattr__(self, attr, value):
        ...
        if not self.__dict__.has_key(
                '_Cursor__initialised'):  # this test allows attributes to be set in the __init__ method
            return object.__setattr__(self, attr, value)
...
def register_ops(module, connection):
    ...
    for f in module.__dict__:
        ...
        if hasattr(fobject, 'registered') and type(
                fobject.registered).__name__ == 'bool' and fobject.registered == True:
            opname = f.lower()

            if firstimport:
                if opname != f:
                    raise MadisError(
                        "Extended SQLERROR: Function '" + module.__name__ + '.' + f + "' uses uppercase characters. Functions should be lowercase")

                if opname.upper() in sqltransform.sqlparse.keywords.KEYWORDS:
                    raise MadisError(
                        "Extended SQLERROR: Function '" + module.__name__ + '.' + opname + "' is a reserved SQL function")
            ...
            if type(fobject).__name__ == 'function':
                if opexists(opname):
                    raise MadisError(
                        "Extended SQLERROR: Row operator '" + module.__name__ + '.' + opname + "' name collision with other operator")
            ...
            if type(fobject).__name__ == 'classobj':
                if opexists(opname):
                    raise MadisError(
                        "Extended SQLERROR: Aggregate operator '" + module.__name__ + '.' + opname + "' name collision with other operator")
