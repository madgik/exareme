...
def createConnection(db):
    ...
    try:
        connection.cursor().execute(
            "PRAGMA temp_store_directory='.';PRAGMA page_size=16384;PRAGMA default_cache_size=3000;")
        # if pipedinput:
    except:
        pass
    ...
    if t in alltablescompl and t not in updated_tables:
        ...
        try:
            if '.' in t:
                ...
                tname = t
            cursor = connection.cursor()
            cexec = cursor.execute(
                'select * from ' + dbname + ".sqlite_master where type='index' and tbl_name='" + str(tname) + "'")
...
def update_cols_from_tables_in_text(t):
    ...
    stablesreg = '|'.join(
        (x.replace('$', '\$').replace('.', '\.') for x in sorted(alltablescompl, key=len, reverse=True)))
...
def mcomplete(textin, state):
    ...
    # Detect if in simplified 'from' or .schema
            ...
            beforecompl, re.DOTALL | re.UNICODE):
    ...
    if hits == [] and text[-2:] != '..':
        ...
        if prepost:
            ...
            hits = [x.lower() for x in lastcols + [y for y in colscompl if y.find('.') == -1] if
                    x.lower()[:len(text)] == unicode(text.lower())]
...
def schemaprint(cols):
    ...
    if cols != []:
        sys.stdout.write(
            Style.BRIGHT + '--- ' + Style.NORMAL + Fore.RED + '[' + Style.BRIGHT + '0' + Style.NORMAL + '|' + Style.RESET_ALL + Style.BRIGHT + 'Column names ' + '---' + Style.RESET_ALL + '\n')
        colschars = 0
        i1 = 1
        for i in cols:
        ...
        if colschars <= 80:
            ...
            for i in cols:
                sys.stdout.write(
                    Fore.RED + '[' + Style.BRIGHT + str(i1) + Style.NORMAL + '|' + Style.RESET_ALL + i + ' ')
                i1 += 1
            sys.stdout.write('\n')
        else:
...
sqlandmtermstatements = ['select ', 'create ', 'where ', 'table ', 'group by ', 'drop ', 'order by ', 'index ', 'from ',
                         'alter ', 'limit ', 'delete ', '..',
...
while True:
            ...
            if not argument:
                ...
                if argument not in alltables:
                    ...
                    if '.' in argument:
                        ...
                        db = sa[0]
                        argument = ''.join(sa[1:])
                    statement = "select sql from (select * from " + db + ".sqlite_master union all select * from sqlite_temp_master) where tbl_name like '%s' and sql is not null;" % (
                        argument)
    ...
    if statement:
        ...
        try:
            ...
            if not pipedinput:
                if rownum == 0:
                    printterm("Query executed in %s min. %s sec %s msec." % ((
