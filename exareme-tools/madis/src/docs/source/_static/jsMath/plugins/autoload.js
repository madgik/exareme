...
if (jsMath.Autoload == null) {
    jsMath.Autoload = {}
}
jsMath.Add = function (dst, src) {
    for (var id in src) {
        dst[id] = src[id]
    }
};
...
jsMath.Add(jsMath.Autoload, {
    ...
    Script: {
        ...
        Load: function (url) {
            if (this.request && !(this.operaXMLHttpRequestBug && url == 'jsMath.js')) {
                setTimeout(function () {
                    jsMath.Autoload.Script.xmlLoad(url)
                }, 1);
            } else {
                this.startLoad(url);
            }
        ...
        Push: function (name, data) {
            this.queue[this.queue.length] = [name, data]
        },
        RunStack: function () {
            if (this.tex2math) {
                jsMath.Autoload.Check2();
                return
            }
    ...
    afterLoad: function () {
        ...
        //
        if (jsMath.tex2math.window) {
            jsMath.tex2math.window.jsMath = jsMath
        }
        if (jsMath.browser == 'MSIE') {
            window.onscroll = jsMath.window.onscroll
        }
        ;
    ...
    stubs: {
        ...
        ProcessElement: function (data) {
            jsMath.Autoload.Script.Push('ProcessElement', [data])
        },
    ...
    InitStubs: function () {
        jsMath.Add(jsMath, jsMath.Autoload.stubs)
    }
...
if (jsMath.Autoload.findTeXstrings == null) {
    jsMath.Autoload.findTeXstrings = 0
}
if (jsMath.Autoload.findLaTeXstrings == null) {
    jsMath.Autoload.findLaTeXstrings = 0
}

jsMath.Autoload.Script.Init();
jsMath.Autoload.InitStubs();
if (document.body && !jsMath.Autoload.delayCheck) {
