...
jsMath.Box.TeXIMG = function (font, C, size) {
    ...
    var w = (img[0] - img[3]) * scale;
    var h = img[1] * scale;
    var d = -img[2] * scale;
    var x = img[3] - imgFont.x[C % 16];
    var y = img[1] - img[2] - imgFont.y[Math.floor(C / 16)];
    var wh;
    var xy;
    var v;
    var ladjust = "";
    ...
    if ((mustScale || jsMath.Controls.cookie.scaleImg) && !jsMath.Browser.operaImageFonts) {
        ...
        d -= 1 / jsMath.em;
        y += 1;
        x += 1; // try to adjust for rounding errors
        ...
        if (img[3]) {
            ladjust = "margin-left:" + jsMath.HTML.Em(-img[3] * scale * bScale) + ";"
        }
    } else {
        ...
        vadjust = "vertical-align:" + (-img[2]) + "px;";
        v = (img[1] - img[2]) + "px";
        if (img[3]) {
            ladjust = "margin-left:" + (-img[3]) + "px;"
        }
    }

    wadjust = (c.w == null || Math.abs(c.w - w) < .01) ? "" : " margin-right:" + jsMath.HTML.Em(c.w - w) + ';';
    if (img[2] == 0 || jsMath.Browser.valignBug) {
        vadjust = ""
    }
    ...
    if (imgFont.loaded == -1) {
        IMG = "";
        c.img.reload = jsMath.Img.fonts[id]
    }
...
jsMath.Img.AddFont = function (size, def) {
    ...
    ;
    for (var font in def) {
        def[font].x = def[font][128];
        def[font].y = def[font][129];
...
jsMath.Browser.Init = function () {
    ...
    jsMath.Browser.ImgFontInit = function () {
        ...
        if (jsMath.browser == 'Mozilla') {
            if (!jsMath.Browser.VersionAtLeast("3.0")) {
            ...
            if (jsMath.platform == 'mac') {
                ...
                if (!jsMath.Controls.userSet.alpha) {
                    jsMath.Controls.cookie.alpha = 0
                }
...
jsMath.Controls.OperaInit = function () {
    ...
    this.OldMain = this.Main;
    this.Main = this.OperaMain;
    this.OldOptions = this.Options;
    this.Options = this.OperaOptions;
    this.OperaMain(1);
};
