    ...
    if (!document.getElementById || !document.childNodes || !document.createElement) {
        ...
        window.jsMath = {
            ...
            Element: function (name) {
                return jsMath.document.getElementById('jsMath_' + name)
            },
            ...
            Init: function () {
                ...
                if (!cache[this.em]) {
                    cache[this.em] = {};
                    cache[this.em].bb = this.BBoxFor('x');
                    var hh = cache[this.em].bb.h;
                ...
                var bb = cache[this.em].bb;
                var h = bb.h;
                var d = cache[this.em].d
                this.h = (h - d) / this.em;
                this.d = d / this.em;
            ...
            Loaded: function () {
                if (jsMath_old) {
                    ...
                    for (var i = 0; i < override.length; i++) {
                        if (jsMath_old[override[i]]) {
                            delete jsMath_old[override[i]]
                        }
                    }
                }
                if (jsMath_old) {
            ...
            Package: function (obj, def) {
                this.Insert(obj.prototype, def)
            }
                ...
                if (!(jsMath.Controls.cookie.asynch && jsMath.Controls.cookie.progress)) {
                    ...
                    if (!this.request && window.ActiveXObject && !this.mustPost) {
                        ...
                        for (var i = 0; i < xml.length && !this.request; i++) {
                            try {
                                this.request = new ActiveXObject(xml[i])
                            } catch (err) {
                            }
                        }
                    }
...
//    this.debug('xmlRequest: '+url);
                try {
                    ...
                    this.blocking = 0;
                    if (jsMath.Translate.restart && jsMath.Translate.asynchronous) {
                        return ""
                    }
...
//    this.debug('xml Done ['+this.queue.length+' + '+tmpQueue.length+']');
            ...
            Synchronize: function (code, data) {
                if (typeof(code) != 'string') {
                    jsMath.Script.Push(null, code, data)
                }
                else {
                    jsMath.Script.Push(jsMath.window, 'eval', code)
                }
            },
...
//    this.debug('Loading: '+url);
            ...
            startLoad: function (url) {
                ...
                if (this.mustPost) {
                    iframe.src = jsMath.Post.startLoad(url, iframe)
                }
                else if (url.match(/\.js$/)) {
                    iframe.src = jsMath.root + "jsMath-loader.html"
                }
                else {
                    iframe.src = this.url
                }
...
//    this.debug('Starting: ['+this.queue.length+'] '+this.url);
                this.tmpQueue = this.queue;
                this.queue = [];
            },
            End: function () {
//    this.debug('Ending:   ['+this.queue.length+' + '+this.tmpQueue.length+'] '+this.url);
                this.queue = this.queue.concat(this.tmpQueue);
...
//    this.debug('endDelay');
            ...
            WaitForImage: function (file) {
                ...
                img.onload = function () {
                    if (--jsMath.Script.imageCount == 0) jsMath.Script.endDelay()
                }
                img.onerror = img.onload;
                img.onabort = img.onload;
                img.src = file;
            },
        ...
        jsMath.Post = {
            ...
            Listener: function (event) {
                ...
                if (ddomain == null || ddomain == "" || ddomain == "null") {
                    ddomain = "localhost"
                }
             ...
             */
            Commands: {
                SCR: function (message) {
                    jsMath.window.eval(message)
                },
                ERR: function (message) {
                    jsMath.Script.cancelLoad(message, 3000)
                },
                BGN: function (message) {
                    jsMath.Script.Start()
                },
                END: function (message) {
                    if (message) jsMath.Script.End();
        ...
        jsMath.Message = {
            ...
            Set: function (text, canCancel) {
                ...
                if (jsMath.Controls.cookie.progress) {
                    ...
                    if (canCancel) {
                        ...
                        if (!this.message.style.cursor) {
                            this.message.style.cursor = 'hand'
                        }
                    ...
                    if (text.substr(0, 8) != "Loading ") {
                        jsMath.window.status = text
                    }
                }
            },
            ...
            UnBlank: function () {
                if (this.blank) {
                    jsMath.document.body.removeChild(this.blank)
                }
                this.blank = null;
            }
        };
        ...
        jsMath.Setup = {
            ...
            Source: function () {
                ...
                if (jsMath.root.charAt(0) == '\\') {
                    jsMath.root = jsMath.root.replace(/\\/g, '/')
                }
                if (jsMath.root.charAt(0) == '/') {
            ...
            Domain: function () {
                try {
                    jsMath.document.domain
                } catch (err) {
                    return
                }
                var jsDomain = '';
                var pageDomain = jsMath.document.domain;
                if (jsMath.root.match('://([^/]*)/')) {
                    jsDomain = RegExp.$1
                }
            ...
            Styles: function (styles) {
                ...
                if (jsMath.Browser.styleChangeDelay) {
                    jsMath.Script.Push(jsMath.Script, 'Delay', 1)
                }
            },
            ...
            StyleString: function (styles) {
                ...
                var string = '';
                for (id in styleString) {
                    string += id + " {" + styleString[id] + "}\n"
                }
                return string;
            },
            ...
            Body: function () {
                ...
                if (jsMath.Controls.cookie.blank) {
                    jsMath.Message.Blank()
                }
                ;
                this.inited = -4;
             ...
             */
            User: function (when) {
                if (jsMath.Setup.UserEvent[when]) {
                    (jsMath.Setup.UserEvent[when])()
                }
            },
        ...
        jsMath.Browser = {
            ...
            MSIE: function () {
                if (this.spanHeightVaries && !this.spanHeightTooBig) {
                    ...
                    if (jsMath.platform == 'pc') {
                        ...
                        this.buttonCheck = 1;
                        this.msieBlankBug = 1;
                        this.msieAccentBug = 1;
                        this.msieRelativeClipBug = 1;
                        this.msieDivWidthBug = 1;
                        this.msiePositionFixedBug = 1;
                        this.msieIntegralBug = 1;
                        this.waitForImages = 1;
                        this.msieAlphaBug = !this.IE7;
                        this.alphaPrintBug = !this.IE7;
                        ...
                        if (!this.IE7 || this.quirks) {
                            ...
                            jsMath.fixedDiv = jsMath.Setup.DIV("fixedDiv", {
                                position: 'absolute',
                                zIndex: 101
                            });
                        ...
                        jsMath.styles['.tex2math_div'] = {};
                        jsMath.Add(jsMath.styles['.tex2math_div'], jsMath.styles['div.typeset']);
                        ...
                        this.msieAbsoluteBug = 1;
                        this.msieButtonBug = 1;
                        this.msieDivWidthBug = 1;
                        this.msieBlankBug = 1;
                        this.quirks = 1;
            ...
            Mozilla: function () {
                if (jsMath.hidden.ATTRIBUTE_NODE && jsMath.window.directories) {
                    ...
                    if (this.VersionAtLeast("3.0")) {
                        this.mozImageSizeBug = 1
                    }
                }
            },
        ...
        jsMath.Font = {
            ...
            extra_message: 'Extra TeX fonts not found: <b><span id="jsMath_ExtraFonts"></span></b><br/>'
            + 'Using image fonts instead.  This may be slow and might not print well.<br/>\n'
            + 'Use the jsMath control panel to get additional information.',

            print_message: 'To print higher-resolution math symbols, click the<br/>\n'
            ...
            CheckTeX: function () {
                ...
                if (jsMath.browser != 'Mozilla' ||
                    ...
                    if (!jsMath.nofonts) {
                        jsMath.Setup.Script("jsMath-BaKoMa-fonts.js")
                    }
                }
            },
                ...
                if (jsMath.nofonts) {
                    if (cookie.autofont || cookie.font == 'tex') {
                        ...
                        if (cookie.warn) {
                            ...
                            cookie.warn = 0;
                            jsMath.Controls.SetCookie(0);
                            if (jsMath.window.NoFontMessage) {
                                jsMath.window.NoFontMessage()
                            }
                            else {
            ...
            Register: function (data, force) {
                ...
                var name = fontname.replace(/10$/, '');
                var fontfam = jsMath.TeX.fam.length;
                if (data.prefix == null) {
                    data.prefix = ""
                }
                if (!data.style) {
                    data.style = "font-family: " + data.prefix + fontname + ", serif"
                }
                if (!data.styles) {
                    data.styles = {}
                }
                ...
                if (cookie.font == 'unicode' || jsMath.noImgFonts) {
                    if (data.fallback) {
                        data.fallback(fontname, fontfam, data)
                    }
                    return;
                }
                //  Image fonts
            ...
            LoadRegistered: function () {
                var i = 0;
                while (i < this.register.length) {
                    this.Register(this.register[i++], 1)
                }
                this.register = [];
            },
            ...
            URL: function (name) {
                return jsMath.Img.root + name + '/def.js'
            }
        ...
        jsMath.Controls = {
            ...
            Button: function () {
                ...
                if (button.offsetWidth < 30) {
                    button.style.width = "auto"
                }
                if (!this.cookie.button) {
...
//    jsMath.fixedDiv.style.height = document.body.clientHeight + 'px';
            ...
            GetCookie: function () {
                ...
                jsMath.Add(this.defaults, this.cookie);
                this.userSet = {};
            ...
            localSetCookie: function (cookie, warn) {
                ...
                if (cookie != '') {
                    href += '?jsMath=' + escape(cookie)
                }
                if (href != jsMath.window.location.href) {
                    this.Reload(href)
                }
            },
            ...
            Reload: function (url) {
                ...
                this.loaded = 0;
                jsMath.Setup.inited = -100;
                jsMath.Global.ClearCache();
                if (url) {
                    jsMath.window.location.replace(url)
                }
                else {
        ...
        jsMath.Click = {
            ...
            CheckDblClick: function (event) {
                ...
                if (!jsMath.Click.DblClick) {
                    ...
                    var tmpEvent = event;
                    event = {};
                    for (var id in tmpEvent) {
                        event[id] = tmpEvent[id]
                    }
                }
        ...
        jsMath.TeX = {
            ...
            cmmi10: [
                ...
                [0.521, 0.694, 0, {ic: 0.0315}], [0.298, 0.694, 0, {
                    ic: 0.0197,
                    krn: {'127': 0.0833}
                }],
            ...
            cmti10: [
                ...
                [0.818, 0.431, 0, {ic: 0.0767}], [0.562, 0.431, 0, {
                    ic: 0.0767,
                    krn: {'39': -0.102}
                }],
                [0.511, 0.431, 0, {
            ...
            cmbx10: [
                ...
                [0.671, 0.694, 0, {
                    ic: 0.109,
                    krn: {'39': 0.109, '63': 0.109, '33': 0.109, '41': 0.109, '93': 0.109},
                    lig: {'105': 14, '108': 15}
                }],
                ...
                [0.831, 0.444, 0, {
                    ic: 0.016,
                    krn: {'101': -0.0319, '97': -0.0319, '111': -0.0319, '99': -0.0319}
                }],
                [0.607, 0.444],
                [0.607, 0.444, 0.194, {
                    ic: 0.016,
        ...
        jsMath.Img = {
            ...
            w: {
                '50': 6.9, '60': 8.3, '70': 9.7, '85': 11.8, '100': 13.9,
            ...
            UpdateFonts: function () {
                var change = this.update;
                if (!this.loaded) return;
                for (var font in change) {
                    for (var i = 0; i < change[font].length; i++) {
                        var c = change[font][i];
                        if (c == 'all') {
                            for (c in jsMath.TeX[font]) {
                                jsMath.TeX[font][c].img = {}
                            }
                        }
                        else {
                            jsMath.TeX[font][c].img = {}
            ...
            BestSize: function () {
                ...
                for (var i = 1; i < this.fonts.length; i++) {
                    if (w < (this.w[this.fonts[i]] + 2 * m) / 3) {
                        return i - 1
                    }
            ...
            Init: function () {
                if (jsMath.Controls.cookie.print || jsMath.Controls.cookie.stayhires) {
                    ...
                    this.factor *= 3;
                    if (!jsMath.Controls.isLocalCookie || !jsMath.Global.isLocal) {
                        jsMath.Controls.SetCookie(0)
                    }
                    if (jsMath.Browser.alphaPrintBug) {
                        jsMath.Controls.cookie.alpha = 0
                    }
                }
        ...
        jsMath.HTML = {
            ...
            Blank: function (w, h, d, isRule) {
                ...
                if (isRule) {
                    ...
                    if (jsMath.Browser.widthAddsBorder) {
                        w = 0
                    }
                    ;
                }
                ...
                if (jsMath.Browser.mozInlineBlockBug) {
                    d = -h
                }
                if (jsMath.Browser.msieBorderBug && !isRule) {
                    d -= jsMath.d
                }
                if (d) {
                    style += 'vertical-align:' + this.Em(-d)
                }
            ...
            Rule: function (w, h) {
                if (h == null) {
                    h = jsMath.TeX.default_rule_thickness
                }
        ...
        jsMath.Add(jsMath.Box, {
             ...
             */
            Null: function () {
                return new jsMath.Box('null', '', 0, 0, 0)
            },
            ...
            Text: function (text, tclass, style, size, a, d) {
                ...
                if (a == null || a == 1) {
                    box.h = .9 * TeX.M_height
                }
                else {
                    box.h = 1.1 * TeX.x_height + TeX.scale * a
                }
                return box;
            },
            ...
            TeXfallback: function (C, font, style, size) {
                ...
                if (c.img != null) {
                    return this.TeXnonfallback(C, font, style, size)
                }
                if (c.h != null && c.a == null) {
                    c.a = c.h - 1.1 * jsMath.TeX.x_height
                }
                var a = c.a;
                var d = c.d; // avoid Firefox warnings
            ...
            DelimExtendRelative: function (H, c, font, a, nocenter) {
                ...
                if (C.delim.mid) {// braces
                    ...
                    if (dx) {
                        html += jsMath.HTML.Spacer(dx)
                    }
                    y -= top.h + top.d + rep.h;
                    for (i = 0; i < n; i++) {
                        html += jsMath.HTML.Place(ext, -w, y - i * h)
                    }
                    y -= H / 2 - rep.h / 2;
                    for (i = 0; i < n; i++) {
                        html += jsMath.HTML.Place(ext, -w, y - i * h)
                ...
                box.bh = jsMath.TeX[font].h;
                box.bd = jsMath.TeX[font].d;
                return box;
            },
            ...
            DelimExtendAbsolute: function (H, c, font, a, nocenter) {
                ...
                if (C.delim.mid) {// braces
                    ...
                    for (i = 0; i < n; i++) {
                        html += jsMath.HTML.PlaceAbsolute(ext, 0, y + i * h)
                    }
            ...
            Delimiter: function (H, delim, style, nocenter) {
                ...
                var TeX = jsMath.Typeset.TeX(style, size);
                if (!delim) {
                    return this.Space(TeX.nulldelimiterspace)
                }
                ...
                if (Math.abs(box.y) < .0001) {
                    box.y = 0
                }
                if (box.y) {
                    box = jsMath.Box.SetList([box], CFSH[2], size)
                }
                return box;
            },
            ...
            LayoutRelative: function (size, table, align, cspacing, rspacing, vspace, useStrut, addWidth) {
                ...
                for (i = 0; i < table.length; i++) {
                    ...
                    row = table[i];
                    H[i] = useStrut * jsMath.h * scale;
                    D[i] = useStrut * jsMath.d * scale;
                ...
                if (rspacing[table.length] == null) {
                    rspacing[table.length] = 0
                }
                if (bh == unset) {
                    bh = 0
                }
                ...
                for (j = 0; j < W.length; j++) {
                    ...
                    for (i = 0; i < table.length; i++) {
                        ...
                        if (i + 1 < table.length) {
                            y -= Math.max(HD, D[i] + H[i + 1]) + dy + rspacing[i + 1]
                        }
                    }
                ...
                for (i = 0; i < H.length; i++) {
                    y += Math.max(HD, H[i] + D[i]) + rspacing[i + 1]
                }
                h = y / 2 + jsMath.TeX.axis_height;
                var d = y - h;
            ...
            LayoutAbsolute: function (size, table, align, cspacing, rspacing, vspace, useStrut, addWidth) {
                ...
                for (j = 0; j < W.length; j++) {
                    ...
                    for (i = 0; i < table.length; i++) {
                        entry = table[i][j];
                        if (entry && entry.format != 'null') {
                            if (align[j] && align[j] == 'l') {
                                x = 0
                            } else if (align[j] && align[j] == 'r') {
                                x = W[j] - entry.w
                            } else {
                                x = (W[j] - entry.w) / 2
                            }
        ...
        jsMath.Add(jsMath.mItem, {
             ...
             */
            Typeset: function (box) {
                return new jsMath.mItem('ord', {atom: 1, nuc: box})
            },
             ...
             */
            HTML: function (html) {
                return new jsMath.mItem('html', {html: html})
            }
        ...
        jsMath.mList = function (list, font, size, style) {
            ...
            this.data = {
                openI: null, overI: null, overF: null,
        ...
        jsMath.Package(jsMath.mList, {
             ...
             */
            Add: function (box) {
                return (this.mlist[this.mlist.length] = box)
            },
            ...
            Delete: function (i, j) {
                ...
                if (this.mlist.splice) {
                    this.mlist.splice(i, j - i + 1)
                } else {
                    var mlist = [];
                    for (var k = 0; k < this.mlist.length; k++) {
                        if (k < i || k > j) {
                            mlist[mlist.length] = this.mlist[k]
                        }
                    }
                    this.mlist = mlist;
                }
            ...
            Open: function (left) {
                ...
                delete this.data.overI;
                delete this.data.overF;
            ...
            Close: function (right) {
                if (right != null) {
                    right = new jsMath.mItem('boundary', {right: right})
                }
                var atom;
                var open = this.data.openI;
                var over = this.data.overI;
                var from = this.data.overF;
            ...
            Atomize: function (style, size) {
                ...
                for (var i = 0; i < this.mlist.length; i++) {
                    ...
                    if (mitem.type == 'choice') {
                        this.mlist = this.Atomize.choice(this.style, mitem, i, this.mlist);
                        i--
                    }
                ...
                if (mitem && mitem.type == 'bin') {
                    mitem.type = 'ord'
                }
                if (this.mlist.length >= 2 && mitem.type == 'boundary' &&
                    this.mlist[0].type == 'boundary') {
            ...
            AddDelimiters: function (style, size) {
                ...
                var TeX = jsMath.TeX;
                var a = jsMath.Typeset.TeX(style, size).axis_height;
                ...
                var left = this.mlist[0];
                var right = this.mlist[this.mlist.length - 1];
        ...
        jsMath.Add(jsMath.mList.prototype.Atomize, {
            ...
            phantom: function (style, size, mitem) {
                ...
                if (mitem.h) {
                    box.Remeasured();
                    box.html = jsMath.HTML.Spacer(box.w)
                }
                else {
                    box.html = '', box.w = box.Mw = box.mw = 0;
                }
                if (!mitem.v) {
                    box.h = box.d = 0
                }
            ...
            lap: function (style, size, mitem) {
                ...
                if (mitem.lap == 'llap') {
                    box.x = -box.w
                } else if (mitem.lap == 'rlap') {
                    mlist[1] = jsMath.mItem.Space(-box.w)
                } else if (mitem.lap == 'ulap') {
                    box.y = box.d;
                    box.h = box.d = 0
                } else if (mitem.lap == 'dlap') {
                    box.y = -box.h;
                    box.h = box.d = 0
                }
                mitem.nuc = jsMath.Box.SetList(mlist, style, size);
                if (mitem.lap == 'ulap' || mitem.lap == 'dlap') {
                    mitem.nuc.h = mitem.nuc.d = 0
                }
                mitem.type = 'box';
                delete mitem.atom;
            },
...
//    if (surd.h > 0) {t = surd.h} // thickness of rule is height of surd character
            ...
            accent: function (style, size, mitem) {
                ...
                var font = jsMath.TeX.fam[mitem.accent[1]];
                Font = jsMath.TeX[font];
                while (Font[c].n && Font[Font[c].n].w <= u) {
                    c = Font[c].n
                }
                ...
                if (jsMath.Browser.msieAccentBug) {
                    acc.html += jsMath.HTML.Spacer(.1);
                    acc.w += .1;
                    acc.Mw += .1
                }
                if (Font[c].ic || ic) {
                    acc.x += (ic - (Font[c].ic || 0)) * TeX.scale
                }
            ...
            op: function (style, size, mitem) {
                ...
                if (mitem.nuc.type == 'TeX') {
                    ...
                    if (isD && C.n) {
                        mitem.nuc.c = C.n;
                        C = jsMath.TeX[mitem.nuc.font][C.n]
                    }
                ...
                if (!box) {
                    box = mitem.nuc = jsMath.Box.Set(mitem.nuc, style, size).Remeasured()
                }
                if (mitem.limits) {
                    var W = box.w;
                    var x = box.w;
                    ...
                    if (x < W) {
                        mlist[mlist.length] = jsMath.mItem.Space(W - x)
                    }
                    mitem.nuc = jsMath.Box.SetList(mlist, style, size);
                    mitem.nuc.h += dh;
                    mitem.nuc.d += dd;
                    ...
                    if (jsMath.Browser.msieIntegralBug && mitem.sub && C && C.ic) {
                        ...
                        mitem.nuc = jsMath.Box.SetList([box], style, size)
                    }
                    jsMath.mList.prototype.Atomize.SupSub(style, size, mitem);
                }
            },
            ...
            SupSub: function (style, size, mitem) {
                ...
                if (!mitem.sup) {
                    ...
                    mitem.nuc = jsMath.Box.SetList([box, sub], style, size).Styled();
                    delete mitem.sub;
                    return;
                }
                ...
                else if (style.charAt(style.length - 1) == "'") {
                    p = TeX.sup3
                }
                else {
                    p = TeX.sup2
                ...
                if (!mitem.sub) {
                    sup.y = u;
                    mitem.nuc = jsMath.Box.SetList([box, sup], style, size);
                    delete mitem.sup;
                    return;
                }
                ...
                if (sup.w + sup.x > sub.w) {
                    sup.x -= sub.w;
                    mitem.nuc = jsMath.Box.SetList([box, sub, sup], style, size)
                } else {
                    sub.x -= (sup.w + sup.x);
                    mitem.nuc = jsMath.Box.SetList([box, sup, sub], style, size)
                }
        ...
        jsMath.Add(jsMath.Typeset, {
             ...
             *  (see TeXbook, p. 441)
             */
            UpStyle: function (style) {
                return this.upStyle[style]
            },
            DownStyle: function (style) {
                return this.downStyle[style]
            },
            ...
            AddClass: function (tclass, html) {
                if (tclass != '' && tclass != 'normal') {
                    html = jsMath.HTML.Class(tclass, html)
                }
                return html;
            }
        ...
        jsMath.Package(jsMath.Typeset, {
            ...
            GetSeparation: function (l, r, style) {
                if (l && l.atom && r.atom) {
                    var table = this.DTsep;
                    if (style.charAt(0) == "S") {
                        table = this.SSsep
                    }
                    var row = table[l.type];
                    if (row && row[r.type] != null) {
                        return jsMath.TeX[this.sepW[row[r.type]]]
                    }
                }
                return 0;
            },
            ...
            Typeset: function (style, size) {
                    ...
                    switch (mitem.type) {
                        ...
                        case 'space':
                            if (typeof(mitem.w) == 'object') {
                                if (this.style.charAt(1) == 'S') {
                                    mitem.w = .5 * mitem.w[0] / 18
                                }
                                else if (this.style.charAt(0) == 'S') {
                                    mitem.w = .7 * mitem.w[0] / 18
                                }
                                else {
                                    mitem.w = mitem.w[0] / 18
                                }
                            }
                        ...
                        default:   // atom
                            ...
                            if (mitem.nuc.format == 'text') {
                                ...
                                if (this.tbuf == '' && this.cbuf == '') {
                                    this.tx = this.x
                                }
                                this.tbuf += mitem.nuc.html;
                                this.tclass = mitem.nuc.tclass;
                            } else {
                                this.FlushClassed();
                            ...
                            this.h = Math.max(this.h, mitem.nuc.h + mitem.nuc.y);
                            this.bh = Math.max(this.bh, mitem.nuc.bh);
                            this.d = Math.max(this.d, mitem.nuc.d - mitem.nuc.y);
                            this.bd = Math.max(this.bd, mitem.nuc.bd);
                            break;
                    }
                }
                ...
                if (this.dx) {
                    this.hbuf += jsMath.HTML.Spacer(this.dx);
                    this.w += this.dx;
                    if (this.w > this.Mw) {
                        this.Mw = this.w
                    }
            ...
            Place: function (item) {
                ...
                if (item.x) {
                    html += ' margin-left:' + jsMath.HTML.Em(item.x) + ';'
                }
                if (item.y) {
                    html += ' top:' + jsMath.HTML.Em(-item.y) + ';'
                }
            ...
            PlaceSeparateSkips: function (item) {
                ...
                if (item.x) {
                    item.html = jsMath.HTML.Spacer(item.x) + item.html
                }
                item.h += item.y;
                item.d -= item.y;
                item.x = 0;
...
//  angle:      ['Replace','ord','&#x2220;','normal'],
            ...
            Process: function (arg) {
                ...
                if (arg.mlist.Length() == 0) {
                    return null
                }
                if (arg.mlist.Length() == 1) {
                    var atom = arg.mlist.Last();
                    if (atom.atom && atom.type == 'ord' && atom.nuc && !atom.sub && !atom.sup && (atom.nuc.type == 'text' || atom.nuc.type == 'TeX')) {
                        return atom.nuc
                    }
                }
            ...
            GetArgument: function (name, noneOK) {
                ...
                if (this.i >= this.string.length) {
                    if (!noneOK) this.Error("Missing argument for " + name);
                    return null
                }
                if (this.string.charAt(this.i) == this.close) {
                    if (!noneOK) this.Error("Extra close brace");
                    return null
                }
                if (this.string.charAt(this.i) == this.cmd) {
                    this.i++;
                    return this.cmd + this.GetCommand()
                }
                if (this.string.charAt(this.i) != this.open) {
            ...
            GetDelimiter: function (name) {
                ...
                if (this.i < this.string.length) {
                    this.i++;
                    if (c == this.cmd) {
                        c = '\\' + this.GetCommand(name);
                        if (this.error) return null
                    }
                    if (this.delimiter[c] != null) {
            ...
            ParseDimen: function (dimen, name, advance, nomu) {
                ...
                if (!match) {
                    this.Error("Missing dimension or its units for " + name);
                    return null
                }
                if (advance) {
            ...
            GetBrackets: function (name) {
                ...
                while (this.i < this.string.length) {
                    ...
                    if (c == '{') {
                        ...
                        if (pcount == 0) {
                            this.Error("Extra close brace while looking for ']'");
                            return null
                        }
                        pcount--;
                    } else if (c == this.cmd) {
                        ...
                        if (pcount == 0) {
                            return this.string.slice(start, this.i - 1)
                        }
                    }
                }
            ...
            GetUpto: function (name, token) {
                ...
                while (this.i < this.string.length) {
                    ...
                    if (c == '{') {
                        ...
                        if (pcount == 0) {
                            this.Error("Extra close brace while looking for " + this.cmd + token);
                            return null
                        }
                        pcount--;
                    } else if (c == this.cmd) {
                        ...
                        if (this.string.slice(this.i, this.i + 5) == "begin") {
                            pcount++;
                            this.i += 4
                        }
            ...
            ProcessUpto: function (name, token) {
                var arg = this.GetUpto(name, token);
                if (this.error) return null;
                return this.Process(arg);
            },
            ...
            GetEnd: function (env) {
                ...
                var name = '';
                while (name != env) {
                    body += this.GetUpto('begin{' + env + '}', 'end');
                    if (this.error) return null;
                    name = this.GetArgument(this.cmd + 'end');
                    if (this.error) return null;
                }
                return body;
            },
            ...
            Prime: function (c) {
                ...
                if (base.sup) {
                    this.Error("Prime causes double exponent: use braces to clarify");
                    return
                }
                var sup = '';
                while (c == "'") {
            ...
            RaiseLower: function (name) {
                var h = this.GetDimen(this.cmd + name, 1);
                if (this.error) return;
                var box = this.ProcessScriptArg(this.cmd + name);
                if (this.error) return;
                if (name == 'lower') {
                    h = -h
                }
            ...
            MoveLeftRight: function (name) {
                var x = this.GetDimen(this.cmd + name, 1);
                if (this.error) return;
                var box = this.ProcessScriptArg(this.cmd + name);
                if (this.error) return;
                if (name == 'moveleft') {
            ...
            Require: function (name) {
                var file = this.GetArgument(this.cmd + name);
                if (this.error) return;
            ...
            Extension: function (name, data) {
                ...
                if (name != null) {
                    delete jsMath.Parser.prototype[data[1] || 'macros'][name]
                }
             ...
             */
            Frac: function (name) {
                var num = this.ProcessArg(this.cmd + name);
                if (this.error) return;
                var den = this.ProcessArg(this.cmd + name);
                if (this.error) return;
             ...
             */
            Sqrt: function (name) {
                var n = this.GetBrackets(this.cmd + name);
                if (this.error) return;
                var arg = this.ProcessArg(this.cmd + name);
                if (this.error) return;
             ...
             */
            Root: function (name) {
                var n = this.ProcessUpto(this.cmd + name, 'of');
                if (this.error) return;
                var arg = this.ProcessArg(this.cmd + name);
                if (this.error) return;
                var box = jsMath.mItem.Atom('radical', arg);
            ...
            BuildRel: function (name) {
                var top = this.ProcessUpto(this.cmd + name, 'over');
                if (this.error) return;
                var bot = this.ProcessArg(this.cmd + name);
                if (this.error) return;
            ...
            MakeBig: function (name, data) {
                var type = data[0];
                var h = data[1] * jsMath.p_height;
                var delim = this.GetDelimiter(this.cmd + name);
                if (this.error) return;
             ...
             */
            Char: function (name) {
                var font = this.GetArgument(this.cmd + name);
                if (this.error) return;
                var n = this.GetArgument(this.cmd + name);
                if (this.error) return;
                if (!jsMath.TeX[font]) {
                    jsMath.TeX[font] = [];
            ...
            Matrix: function (name, delim) {
                var data = this.mlist.data;
                var arg = this.GetArgument(this.cmd + name);
                if (this.error) return;
            ...
            HandleEntry: function (name) {
                if (!this.matrix) {
                    this.Error(name + " can only appear in a matrix or array");
                    return
                }
                if (this.mlist.data.openI != null) {
                    var open = this.mlist.Get(this.mlist.data.openI);
                    if (open.left) {
                        this.Error("Missing " + this.cmd + "right")
                    }
                    else {
                        this.Error("Missing close brace")
                    }
            ...
            HandleRow: function (name, last) {
                var dimen;
                if (!this.matrix) {
                    this.Error(this.cmd + name + " can only appear in a matrix or array");
                    return
                }
                if (name == "\\") {
                    dimen = this.GetBrackets(this.cmd + name);
                    if (this.error) return;
                    if (dimen) {
                        dimen = this.ParseDimen(dimen, this.cmd + name, 0, 1)
                    }
                }
                ...
                if (dimen) {
                    this.rspacing[this.table.length] = dimen
                }
                this.row = [];
            },
            ...
            HandleNoAlign: function (name) {
                ...
                var d = this.ParseDimen(skip, this.cmd + RegExp.$1, 0, 1);
                if (this.error) return;
             ...
             */
            Begin: function (name) {
                var env = this.GetArgument(this.cmd + name);
                if (this.error) return;
                if (env.match(/[^a-z*]/i)) {
                    this.Error('Invalid environment name "' + env + '"');
                    return
                }
            ...
            Equation: function (name) {
                var arg = this.GetEnd(name);
                if (this.error) return;
                this.string = arg + this.string.slice(this.i);
                this.i = 0;
            },
             ...
             */
            Hskip: function (name) {
                var w = this.GetDimen(this.cmd + name);
                if (this.error) return;
             ...
             */
            HBox: function (name) {
                var text = this.GetArgument(this.cmd + name);
                if (this.error) return;
            ...
            Rule: function (name, style) {
                var w = this.GetDimen(this.cmd + name, 1);
                if (this.error) return;
                var h = this.GetDimen(this.cmd + name, 1);
                if (this.error) return;
                var d = this.GetDimen(this.cmd + name, 1);
                if (this.error) return;
                h += d;
                var html;
                if (h != 0) {
                    h = Math.max(1.05 / jsMath.em, h)
                }
                if (h == 0 || w == 0 || style == "blank") {
                    html = jsMath.HTML.Blank(w, h)
                } else {
            ...
            MathAccent: function (name, accent) {
                var c = this.ProcessArg(this.cmd + name);
                if (this.error) return;
                var atom = jsMath.mItem.Atom('accent', c);
                atom.accent = accent[0];
                this.mlist.Add(atom);
            },
            ...
            Limits: function (name, data) {
                ...
                if (!atom || atom.type != 'op') {
                    this.Error(this.cmd + name + " is allowed only on operators");
                    return
                }
                atom.limits = data[0];
            },
            ...
            Macro: function (name, data) {
                ...
                if (data[1]) {
                    ...
                    for (var i = 0; i < data[1]; i++) {
                        args[args.length] = this.GetArgument(this.cmd + name);
                        if (this.error) return
                    }
                    ...
                    if (c == this.cmd) {
                        ...
                        if (c == "#") {
                            ...
                            if (!c.match(/[1-9]/) || c > args.length) {
                                this.Error("Illegal macro parameter reference");
                                return null
                            }
             ...
             */
            HandleVariable: function (c) {
                this.HandleTeXchar(7, 1, c.charCodeAt(0))
            },
            HandleNumber: function (c) {
                this.HandleTeXchar(7, 0, c.charCodeAt(0))
            },
            ...
            HandleCS: function () {
                ...
                if (this.macros[cmd]) {
                    ...
                    this[macro[0]](cmd, macro.slice(1));
                    return;
                }
            ...
            HandleClose: function () {
                ...
                var open = this.mlist.Get(this.mlist.data.openI);
                if (!open || open.left == null) {
                    this.mlist.Close()
                }
                else {
                    this.Error("Extra close brace or missing " + this.cmd + "right");
                    return
                }
            },
            ...
            HandleLeft: function (name) {
                var left = this.GetDelimiter(this.cmd + name);
                if (this.error) return;
            ...
            HandleRight: function (name) {
                var right = this.GetDelimiter(this.cmd + name);
                if (this.error) return;
                var open = this.mlist.Get(this.mlist.data.openI);
                if (open && open.left != null) {
                    ...
                    this.Error("Extra open brace or missing " + this.cmd + "left");
                }
            },
            ...
            HandleOver: function (name, data) {
                if (this.mlist.data.overI != null) {
                    this.Error('Ambiguous use of ' + this.cmd + name);
                    return
                }
                ...
                if (data.length > 0) {
                    ...
                    this.mlist.data.overF.left = this.GetDelimiter(this.cmd + name);
                    if (this.error) return;
                    this.mlist.data.overF.right = this.GetDelimiter(this.cmd + name);
                    if (this.error) return;
                } else {
            ...
            HandleSuperscript: function () {
                var base = this.mlist.Last();
                if (this.mlist.data.overI == this.mlist.Length()) {
                    base = null
                }
                if (base == null || (!base.atom && base.type != 'box' && base.type != 'frac')) {
                    base = this.mlist.Add(jsMath.mItem.Atom('ord', {type: null}))
                ...
                if (base.sup) {
                    if (base.sup.isPrime) {
                        ...
                        this.Error("Double exponent: use braces to clarify");
                        return
                    }
                }
                base.sup = this.ProcessScriptArg('superscript');
                if (this.error) return;
            },
            ...
            HandleSubscript: function () {
                ...
                if (base.sub) {
                    this.Error("Double subscripts: use braces to clarify");
                    return
                }
                base.sub = this.ProcessScriptArg('subscript');
                if (this.error) return;
            },
            ...
            Parse: function () {
                ...
                while (this.i < this.string.length) {
                    c = this.string.charAt(this.i++);
                    if (this.mathchar[c]) {
                        this.HandleMathCode(c, this.mathchar[c])
                    }
                    else if (this.special[c]) {
                        this[this.special[c]](c)
                    }
                    else if (this.letter.test(c)) {
                        this.HandleVariable(c)
                    }
                    else if (this.number.test(c)) {
                        this.HandleNumber(c)
                    }
                    else {
                        this.HandleOther(c)
            ...
            Typeset: function () {
                ...
                if (this.error) {
                    return '<span class="error">' + this.error + '</span>'
                }
                if (box.format == 'null') {
                    return ''
                }
                ...
                box.Styled().Remeasured();
                var isSmall = 0;
                var isBig = 0;
                if (box.bh > box.h && box.bh > jsMath.h + .001) {
                    isSmall = 1
                }
                if (box.bd > box.d && box.bd > jsMath.d + .001) {
                    isSmall = 1
                }
        ...
        jsMath.Extension = {
            ...
            MathChar: function (font, defs) {
                ...
                for (var c in defs) {
                    mathchardef[c] = [defs[c][0], fam, defs[c][1]]
                }
            },
            ...
            URL: function (file) {
                ...
                if (!file.match(/^([a-z]+:|\/|fonts|extensions\/)/i)) {
        ...
        jsMath.Translate = {
            ...
            Parse: function (style, s, noCache) {
                ...
                if (!HTML || noCache) {
                    ...
                    HTML = parse.Typeset();
                    if (!noCache) {
                        cache[jsMath.em][s] = HTML
                    }
                }
                return HTML;
            },

            TextMode: function (s, noCache) {
                this.Parse('T', s, noCache)
            },
            DisplayMode: function (s, noCache) {
                this.Parse('D', s, noCache)
            },
            ...
            GetMathElements: function (obj) {
                ...
                if (typeof(obj) == 'string') {
                    obj = jsMath.document.getElementById(obj)
                }
            ...
            Cancel: function () {
                ...
                if (jsMath.Script.cancelTimer) {
                    jsMath.Script.cancelLoad()
                }
            }
        ...
        jsMath.Add(jsMath, {
            ...
            ConvertTeX: function (element) {
                jsMath.Script.Push(jsMath.tex2math, 'ConvertTeX', element)
            },
            ConvertTeX2: function (element) {
                jsMath.Script.Push(jsMath.tex2math, 'ConvertTeX2', element)
            },
            ConvertLaTeX: function (element) {
                jsMath.Script.Push(jsMath.tex2math, 'ConvertLaTeX', element)
            },
            ConvertCustom: function (element) {
                jsMath.Script.Push(jsMath.tex2math, 'ConvertCustom', element)
            },
            CustomSearch: function (om, cm, od, cd) {
                jsMath.Script.Push(null, function () {
                    jsMath.tex2math.CustomSearch(om, cm, od, cd)
                })
            },
            tex2math: {
                ConvertTeX: function () {
