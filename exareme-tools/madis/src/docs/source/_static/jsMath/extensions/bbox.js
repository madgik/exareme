...
jsMath.Add(jsMath.HTML, {
    ...
    BBox: function (w, h, d, c, s) {
        ...
        if (jsMath.Browser.msieInlineBlockFix) {
            d -= jsMath.d
        }
        if (d) {
...
jsMath.Add(jsMath.mList.prototype.Atomize, {
    ...
    bbox: function (style, size, mitem, prev, mlist) {
        ...
        if (box == null) {
            ...
            h = nuc.h;
            d = nuc.d
        } // values before super/subs-cript
        var nuc = mitem.nuc;
        nuc.Styled();
        var pad = mitem.pad;
        if (pad) {
        ...
        nuc.h = Math.max(nuc.h, h);
        nuc.d = Math.max(nuc.d, d);
        nuc.bh = Math.max(nuc.bh, nuc.h);
        nuc.bd = Math.max(nuc.bd, nuc.d);
        mitem.type = 'ord';
...
jsMath.Package(jsMath.Parser, {
     ...
     */
    BBox: function (name) {
        var extra = this.GetBrackets(this.cmd + name);
        if (this.error) return;
        var arg = this.GetArgument(this.cmd + name);
        if (this.error) return;
        var nuc = this.Process(arg);
        if (this.error) return;
        var color;
        var pad = 0;
        var style = '';
