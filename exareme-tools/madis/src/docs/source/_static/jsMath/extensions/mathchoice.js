...
jsMath.Add(jsMath.mList.prototype.Atomize, {
    ...
    choice: function (style, mitem, i, mlist) {
        ...
        if (M.type == 'mlist') {
            ...
            for (k = i + 1; k < mlist.length; k++) {
                nlist[nlist.length] = mlist[k]
            }
            return nlist;
        } else {
...
jsMath.Package(jsMath.Parser, {
    ...
    MathChoice: function (name) {
        var D = this.ProcessArg(this.cmd + name);
        if (this.error) return;
        var T = this.ProcessArg(this.cmd + name);
        if (this.error) return;
        var S = this.ProcessArg(this.cmd + name);
        if (this.error) return;
        var SS = this.ProcessArg(this.cmd + name);
        if (this.error) return;
