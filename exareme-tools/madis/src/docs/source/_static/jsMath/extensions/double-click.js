...
jsMath.Add(jsMath.Click, {
    ...
    Init: function () {
        if (this.oldStyles) {
            jsMath.Insert(this.styles, this.oldStyles)
        }
    ...
    DblClick: function (data) {
        ...
        var x = Math.floor(event.x - w / 2);
        var y = Math.floor(event.y - h / 2);
        ...
        jsMath.Click.left = x + event.X;
        jsMath.Click.top = y + event.Y;
        jsMath.Click.w = w;
        jsMath.Click.h = source.offsetHeight;
    ...
    Event: function (event) {
        ...
        var X = jsMath.window.pageXOffset;
        var Y = jsMath.window.pageYOffset;
        if (X == null) {
    ...
    StartDragging: function (event) {
        ...
        if (jsMath.Click.dragging) {
            jsMath.Click.StopDragging(event)
        }
        var event = jsMath.Click.Event(event);
    ...
    DragSource: function (event) {
        ...
        if (jsMath.Browser.buttonCheck && !event.button) {
            return jsMath.Click.StopDragging(event)
        }
