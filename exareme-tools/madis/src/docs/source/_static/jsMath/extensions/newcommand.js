...
jsMath.Package(jsMath.Parser, {
    ...
    NewCommand: function (name) {
        ...
        var n = this.trimSpaces(this.GetBrackets(this.cmd + name));
        if (this.error) return;
        ...
        if (cs.charAt(0) == this.cmd) {
            cs = cs.substr(1)
        }
        if (!cs.match(/^(.|[a-z]+)$/i)) {
            this.Error("Illegal control sequence name for " + this.cmd + name);
            return
        }
        if (n != null && !n.match(/^[0-9]+$/)) {
            this.Error("Illegal number of parameters specified in " + this.cmd + name);
            return
        }
    ...
    NewEnvironment: function (name) {
        var env = this.trimSpaces(this.GetArgument(this.cmd + name));
        if (this.error) return;
        var n = this.trimSpaces(this.GetBrackets(this.cmd + name));
        if (this.error) return;
        var bdef = this.GetArgument(this.cmd + name);
        if (this.error) return;
        var edef = this.GetArgument(this.cmd + name);
        if (this.error) return;
        if (n == '') {
            n = null
        }
    ...
    MacroDef: function (name) {
        var cs = this.GetCSname(this.cmd + name);
        if (this.error) return;
        var params = this.GetTemplate(this.cmd + name);
        if (this.error) return;
        var def = this.GetArgument(this.cmd + name);
        if (this.error) return;
    ...
    GetCSname: function (cmd) {
        var c = this.GetNext();
        if (c != this.cmd) {
            this.Error(cmd + " must be followed by a control sequence");
            return null
        }
    ...
    GetTemplate: function (cmd) {
        ...
        while (this.i < this.string.length) {
            ...
            if (c == '#') {
                ...
                if (!c.match(/[1-9]/)) {
                    this.Error("Illegal use of # in " + cmd);
                    return null
                }
                if (1 * c != ++n) {
                    this.Error("Parameters must be numbered sequentially");
                    return null
                }
                i = this.i + 1;
            } else if (c == '{') {
    ...
    MacroWithTemplate: function (name, data) {
        ...
        if (n) {
            ...
            if (params[0] && !this.MatchParam(params[0])) {
                this.Error("Use of " + this.cmd + name + " doesn't match its definition");
                return
            }
            for (var i = 0; i < n; i++) {
    ...
    Environment: function (name, data) {
        ...
        if (n) {
            var args = [];
            for (var i = 0; i < n; i++) {
                args[args.length] = this.GetArgument(this.cmd + "begin{" + name + "}");
                if (this.error) return;
            }
            bdef = this.SubstituteArgs(args, bdef);
        }
        var text = this.GetEnd(name);
        if (this.error) return;
