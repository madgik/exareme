...
def test_valueEnumerationsParameter():
    ...
    assert (
        r.text
        == '{"result" : [{"data":"The value \'abcd\' of the parameter \'encodingparameter\' is not included in the valueEnumerations [dummycoding, sumscoding, simplecoding] .","type":"text/plain+error"}]}'
    )
...
def test_parameter_max_value():
    ...
    assert (
        r.text
        == '{"result" : [{"data":"The value(s) of the parameter \'sstype\' should be less than 3.0 .","type":"text/plain+error"}]}'
    )
