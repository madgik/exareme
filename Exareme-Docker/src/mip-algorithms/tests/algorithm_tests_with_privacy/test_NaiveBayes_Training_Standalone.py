...
class TestNaiveBayesStandalone(unittest.TestCase):
    ...
    def test_NaiveBayesStandalone_1(self):
        ...
        resultsComparison(
            ...
            json.loads(str(self.Test1Result))[0],
    ...
    def test_NaiveBayesStandalone_2(self):
        ...
        data = [
            ...
            {
                "name": "x",
                "value": "rightmtgmiddletemporalgyrus,leftttgtransversetemporalgyrus",
            },
            {"name": "y", "value": "alzheimerbroadcategory"},
            {"name": "alpha", "value": "0.1"},
            {"name": "filter", "value": ""},
        ...
        resultsComparison(
            result["result"][0]["data"]["data"],
            json.loads(str(self.Test2Result))[0],
            "alzheimerbroadcategory",
        )
        ...
        if rowExareme[0] == y:  # Compare a-priori probabilities
            for rowR in jsonRResult2:
                if rowR["Y"] == rowExareme[2]:
                    ...
                    assert math.isclose(
                        ...
                        rel_tol=0,
                        abs_tol=10
                        ** (-abs(Decimal(str(rowR["Freq"])).as_tuple().exponent)),
                    )
                    no = no + 1
            ...
            for rowR in jsonRResult1:
                if rowR["_row"] == rowExareme[2]:
                    ...
                    assert math.isclose(
                        float(rowExareme[3]),
                        float(rowR[str(rowExareme[0]) + ".1"]),
                        rel_tol=0,
                        abs_tol=10
                        ** (
                            -abs(
                                Decimal(str(rowR[str(rowExareme[0]) + ".1"]))
                                .as_tuple()
                                .exponent
                            )
                        ),
                    ...
                    assert math.isclose(
                        float(rowExareme[4]),
                        float(rowR[str(rowExareme[0]) + ".2"]),
                        rel_tol=0,
                        abs_tol=10
                        ** (
                            -abs(
                                Decimal(str(rowR[str(rowExareme[0]) + ".2"]))
                                .as_tuple()
                                .exponent
                            )
                        ),
