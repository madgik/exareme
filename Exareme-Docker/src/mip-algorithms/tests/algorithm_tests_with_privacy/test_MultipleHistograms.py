...
class TestHistogram(unittest.TestCase):
    ...
    def test_Histogram_1(self):
        ...
        data = [
            {"name": "y", "value": "rightententorhinalarea,righthippocampus"},
    ...
    def test_Histogram_2(self):
        ...
        data = [
            ...
            {
                "name": "filter",
                "value": '{"condition": "AND", "rules": [{"id": "rightententorhinalarea","field": "rightententorhinalarea","type": "double", "input": "number", "operator": "is_not_null", "value": null}],"valid": true}',
            },
        ]
        ...
        for j in range(len(result["result"])):
            if Rtest["1"][0] == result["result"][j]["data"]["title"]["text"]:
                ...
                print(
                    "YEEEEES",
                    Rtest["1"][0],
                    result["result"][j]["data"]["title"]["text"],
                )
                ...
                for k in range(len(result["result"][j]["data"]["series"])):
                    ...
                    if variableIsCategorical == 0:
                        if ykey in Rtest:
                            for z in range(len(Rtest[ykey])):
                                if int(Rtest[ykey][z]) > PRIVACY_MAGIC_NUMBER:
                                    assert int(Rtest[ykey][z]) == int(
                                        result["result"][j]["data"]["series"][k][
                                            "data"
                                        ][z]
                                    )
                                else:
                                ...
                                minmaxvalues = result["result"][j]["data"]["xAxis"][
                                    "categories"
                                ][z].split("-")
                                # print (minmaxvalues)
                                assert math.isclose(
                                    float(minmaxvalues[0]),
                                    Rtest["xmax"][z],
                                    rel_tol=0,
                                    abs_tol=10
                                    ** (
                                        -abs(
                                            Decimal(str(Rtest["xmax"][z]))
                                            .as_tuple()
                                            .exponent
                                        )
                                    ),
                                ...
                                assert math.isclose(
                                    float(minmaxvalues[1]),
                                    Rtest["xmin"][z],
                                    rel_tol=0,
                                    abs_tol=10
                                    ** (
                                        -abs(
                                            Decimal(str(Rtest["xmin"][z]))
                                            .as_tuple()
                                            .exponent
                                        )
                                    ),
                            ...
                            for z in range(
                                len(result["result"][j]["data"]["series"][k]["data"])
                            ):
                                assert (
                                    int(
                                        result["result"][j]["data"]["series"][k][
                        ...
                        print(
                            "Categories",
                            result["result"][j]["data"]["xAxis"]["categories"],
                        )
                        print("Key", ykey)
                        for z in range(
                            len(result["result"][j]["data"]["xAxis"]["categories"])
                        ):
                            ykeynew = (
                                ykey
                                + "_"
                                + result["result"][j]["data"]["xAxis"]["categories"][z]
                            )
                            print(ykeynew)
                            if ykeynew in Rtest:
                                print(
                                    Rtest[ykeynew],
                                    result["result"][j]["data"]["series"][k]["data"][z],
                                )
                                if int(Rtest[ykeynew][0]) > PRIVACY_MAGIC_NUMBER:
                                    assert int(Rtest[ykeynew][0]) == int(
                                        result["result"][j]["data"]["series"][k][
                                            "data"
                                        ][z]
                                    )
