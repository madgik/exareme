...
class TestTTESTIndependent(unittest.TestCase):
    ...
    def test_UnpairedTtest_1a(self):
        ...
        print(r.text)
        resultsComparison(
            data, result["result"][0]["data"], json.loads(self.Test1aResult)
        )
    ...
    def test_UnpairedTtest_1b(self):
        ...
        print(r.text)
        resultsComparison(
            data, result["result"][0]["data"], json.loads(self.Test1bResult)
        )

    def test_UnpairedTtest_2(self):
        logging.info(
            "---------- TEST 2: We check if the mean volumes are greater for men than for women. "
        )
        data = [
            {"name": "y", "value": "lefthippocampus"},
            {"name": "x", "value": "gender"},
