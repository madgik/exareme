...
def test_NAIVEBAYES_1():
    ...
    data1 = [
        ...
        {
            "name": "x",
            "value": "car_buying,car_maint,car_doors,car_persons,car_lug_boot,car_safety",
        },
    ...
    data2.append(
        {"name": "dbIdentifier", "value": result1["result"][0]["dbIdentifier"]}
    )
    ...
    corr_trainingResult = [
        ...
        {
            "val": "big",
            "probability": 0.354077956,
            "sigma": "NA",
            "average": "NA",
            "colname": "car_lug_boot",
            "classval": "acc",
        },
        {
            "val": "med",
            "probability": 0.350802489,
            "sigma": "NA",
            "average": "NA",
            "colname": "car_lug_boot",
            "classval": "acc",
    ...
    data3.append(
        {"name": "dbIdentifier", "value": trainingResult["result"][0]["dbIdentifier"]}
    )
    data3.append(
        {"name": "model", "value": json.dumps(trainingResult["result"][0]["data"])}
    )
    ...
    corr_testingResult = [
        {"Reference": "acc", "Prediction": "acc", "val": 59},
...
def check_testingresult(myresult, corr_variable_data):
    ...
    for j in corr_variable_data:
        print(myresult, j)
        if (
            myresult["actualclass"] == j["Reference"]
            and myresult["predictedclass"] == j["Prediction"]
        ):
            exist = True
...
def check_trainingresult(myresult, corr_variable_data):
    ...
    for j in corr_variable_data:
        if (
            myresult["colname"] == j["colname"]
            and myresult["val"] == j["val"]
            and myresult["classval"] == j["classval"]
        ):
            exist = True
            assert math.isclose(
                float(myresult["probability"]),
                float(j["probability"]),
                rel_tol=0,
                abs_tol=10
                ** (-abs(Decimal(str(float(j["probability"]))).as_tuple().exponent)),
            )
    assert exist
