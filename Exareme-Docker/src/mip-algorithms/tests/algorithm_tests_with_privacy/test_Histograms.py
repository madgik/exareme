...
class TestHistogram(unittest.TestCase):
    ...
    def test_Histogram_1(self):
        ...
        data = [
            {"name": "x", "value": "rightententorhinalarea"},
            {"name": "y", "value": ""},
            {"name": "bins", "value": "35"},
    ...
    for d in range(len(jsonRResult)):
        for key in jsonRResult[d]:
            ...
            if key == "xmin" or key == "xmax":
                assert math.isclose(
                    jsonExaremeResult[d][key],
                    jsonRResult[d][key],
                    rel_tol=0,
                    abs_tol=10
                    ** (-abs(Decimal(str(jsonRResult[d][key])).as_tuple().exponent)),
                )
            elif key == "y":
                ...
                for i in range(len(exaremejson)):
                    if grouping in exaremejson[i]:
                        if int(jsonRResult[d][key]) >= minNumberOfData:
                            assert int(exaremejson[i][grouping]) == int(
                                jsonRResult[d][key]
                            )
                        else:
                            assert int(exaremejson[i][grouping]) == 0
                        print(
                            "KEY3:", key, exaremejson[i][grouping], jsonRResult[d][key]
                        )
