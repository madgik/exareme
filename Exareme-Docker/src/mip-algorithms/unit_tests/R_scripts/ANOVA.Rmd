...
# LOAD BALANCED DATA
rawDataB <- read_delim("../unit_tests/data/dementia/data_ANOVA_Balanced_with_inter_V1V2.csv", ",") %>%
  mutate(
...
rawDataUB <- read_delim("../unit_tests/data/dementia/data_ANOVA_Unbalanced_with_inter_V1V2.csv", ",")
rawDataUB <- rawDataUB %>%
  mutate(
    var_I1 = as.factor(var_I1),
    var_I2 = as.factor(var_I2),
...
df0 <- read_csv("../unit_tests/data/dementia/dataset_0.csv") %>%
  mutate(
