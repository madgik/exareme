...
endpointUrl = 'http://localhost:9090/mining/query/PEARSON_CORRELATION'
...
def test_PearsonCorrlation_MIP_AlgoTesting_4():
    ...
    check_result(
            result['result'][0]['data'][1], 'righthippocampus ~ lefthippocampus', 0.902, '< 0.001', 0.889, 0.913
    )
    check_result(
            result['result'][0]['data'][2], 'righthippocampus ~ leftententorhinalarea', 0.808, '< 0.001', 0.784, 0.829
    )
    check_result(
            result['result'][0]['data'][5], 'lefthippocampus ~ leftententorhinalarea', 0.806, '< 0.001', 0.782, 0.828
    )
...
def test_PearsonCorrlation_MIP_AlgoTesting_5():
    ...
    check_result(
            result['result'][0]['data'][2], 'righthippocampus ~ opticchiasm', 0.198, '< 0.001', 0.135, 0.259
    )
    check_result(
            result['result'][0]['data'][5], 'lefthippocampus ~ opticchiasm', 0.211, '< 0.001', 0.148, 0.272
    )
...
def check_result(my_result, r_var_pair, r_corr, r_pval, r_ci_lo, r_ci_hi):
    ...
    assert set(var_pair.replace(' ', '').split('~')) == set(r_var_pair.replace(' ', '').split('~'))
