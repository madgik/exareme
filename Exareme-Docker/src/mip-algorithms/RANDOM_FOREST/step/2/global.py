...
from RF_lib import RFIter2_Loc2Glob_TD, RF_Glob2Loc_TD, RF_step_2_global
...
def main():
    ...
    rF_activePaths = RFIter2_Loc2Glob_TD.load(path.abspath(args.local_step_dbs)).get_data()

    #2. Run algorithm: -->pass
    rF_activePaths = RF_step_2_global(global_state['args_Y'], global_state['args_n_trees'], global_state['rF_DataSchema'], global_state['rF_categoricalVariables'], rF_activePaths, global_state['rF_Logs'])
    #activePaths = cart_step_2_global(global_state['args_X'], global_state['args_Y'], global_state['CategoricalVariables'], activePaths)

    #3. Save global state
    global_state1 = StateData(  stepsNo = global_state['stepsNo'],
