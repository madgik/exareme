    ...
    for i in xrange(len(var_list)):
        ...
        if not is_categorical:
            ...
            if nn < PRIVACY_MAGIC_NUMBER:
                raise PrivacyError(
                    "Removing missing values results in illegal number of datapoints in local db."
                )
            sx = xm.sum()
            sxx = (xm * xm).sum()
            xmin = xm.min()
            xmax = xm.max()
            transf_var = TransferVariable(
                is_categorical, var_name, nn, sx, sxx, xmin, xmax
            )
...
def main():
    ...
    parser.add_argument(
        "-db_query", required=True, help="Query to be executed on local db."
    )
    ...
    # Get data
    var_names = list(args.x.replace(" ", "").split(","))
    ...
    for idx in idx_X:
        ...
        cur.execute(
            "select isCategorical from metadata where code = '" + var_name + "';"
        )
        is_categorical = cur.fetchall()[0][0]
        if is_categorical:
            cur.execute(
                "select enumerations from metadata where code = '" + var_name + "';"
            )
