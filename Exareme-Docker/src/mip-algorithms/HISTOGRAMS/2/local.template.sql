...
select '' as grouping,id, val,minval ,maxval ,num from (histogram metadata:%{x_metadata} bins:%{bins} select %{x} from defaultDB.localinputtblflat)
where  "%{y}"= "";
...
from (histogram metadata:%{x_metadata} bins:%{bins} select %{x} from defaultDB.localinputtblflat where %{ynew}="?")
