...
from mipframework import (
    Algorithm, AlgorithmResult, TabularDataResource,
    create_runner)
...
class LogisticRegression(Algorithm):
    ...
    def global_init(self):
        ...
        # Init model  # todo extract method and then make function
        ll = - 2 * n_obs * np.log(2)
    ...
    def local_final(self):
        ...
        for i, thr in enumerate(thresholds):
            ...
            # todo tp = np.sum(1 if yi==yhi==1 else 0 for yi, yhi in zip(Y,
            #  yhat)) etc..
    ...
    def global_final(self):
        ...
        table2 = TabularDataResource(
            ...
            title='Logistic Regression Summary').render()
        hc_confmat = ConfusionMatrix(title='Confusion Matrix',
                                     confusion_matrix=confusion_mat).render()
        hc_roc = ROC(title='ROC Curve', roc_curve=roc_curve, auc=auc,
                     gini=gini).render()
        self.result = AlgorithmResult(raw_data=raw_data, tables=[table1, table2],
    ...
    def compute_roc(self):
        ...
        auc = 0.0  # todo auc = scipy.integrate.romb(tp_rate, fp_rate) or simps (
        #             romb with 2^k + 1 samples
        for t in range(1, len(fp_rate)):
            auc += (0.5 * (fp_rate[t] - fp_rate[t - 1])
                    * (tp_rate[t] + tp_rate[t - 1]))
        gini = 2 * auc - 1
...
if __name__ == '__main__':
    ...
    runner = create_runner(for_class='LogisticRegression',
                           ...
                           alg_type='iterative', num_workers=3,
