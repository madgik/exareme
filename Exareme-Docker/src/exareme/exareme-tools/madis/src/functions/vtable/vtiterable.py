...
class SourceVT:
    ...
    def Create(self, db, modulename, dbname, tablename, *args):
        ...
        self.tableObjs[tablename] = LTable(self.tableCl, self.tableObjs, self.boolargs, self.nonstringargs,
                                           self.needsescape, self.notsplit, self.staticschema, *args, **dictargs)
...
class LTable:  ####Init means setschema and execstatus
    ...
    @echocall
    def __init__(self, vtable, tblist, boolargs, nonstringargs, needsescape, notsplit, staticschema, *args,
...
def schemastr(tablename, colnames, typenames=None):
    ...
    if not typenames:
        ...
        stripedtypenames = [
            '' if el.lower() == "none" else el if onlyalphnum.match(el) else '"' + el.replace('"', '""') + '"' for el in
            typenames]
