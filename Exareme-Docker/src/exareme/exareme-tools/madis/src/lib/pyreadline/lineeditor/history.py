...
class LineHistory(object):
    ...
    def reverse_search_history(self, searchfor, startpos=None):
        ...
        if _ignore_leading_spaces:
            res = [(idx, line.lstrip()) for idx, line in enumerate(self.history[startpos:0:-1]) if
                   line.lstrip().startswith(searchfor.lstrip())]
        else:
    ...
    def forward_search_history(self, searchfor, startpos=None):
        if startpos is None:
            startpos = self.history_cursor
        if _ignore_leading_spaces:
            res = [(idx, line.lstrip()) for idx, line in enumerate(self.history[startpos:]) if
                   line.lstrip().startswith(searchfor.lstrip())]
        else:
