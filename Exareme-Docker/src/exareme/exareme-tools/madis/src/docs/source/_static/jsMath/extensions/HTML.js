...
jsMath.Package(jsMath.Parser, {
     ...
     */
    Color: function (name) {
        var color = this.GetArgument(this.cmd + name);
        if (this.error) return;
        this.CheckHTML(color, name);
        if (this.error) return;
     ...
     */
    Href: function (name) {
        var href = this.GetArgument(this.cmd + name);
        if (this.error) return;
        this.CheckHTML(href, name);
        if (this.error) return;
     ...
     */
    Class: function (name) {
        var clss = this.GetArgument(this.cmd + name);
        if (this.error) return;
        this.CheckHTML(clss, name);
        if (this.error) return;
     ...
     */
    Style: function (name) {
        var style = this.GetArgument(this.cmd + name);
        if (this.error) return;
        this.CheckHTML(style, name);
        if (this.error) return;
     ...
     */
    CSSId: function (name) {
        var id = this.GetArgument(this.cmd + name);
        if (this.error) return;
        this.CheckHTML(id, name);
        if (this.error) return;
    ...
    AddHTML: function (name, params) {
        ...
        for (var i = 0; i < arg.mlist.Length(); i++) {
            this.mlist.Add(arg.mlist.Get(i))
        }
