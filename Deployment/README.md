...
All nodes should have python installed beforehand, so playbooks can run in every node
```
sudo apt-get install python
```

Changes need to be done in order to run the playbooks. 
```
...
FOLDER_PATH(where mip-federation is TODO pwd in ansible),DESTINATION_PATH (path in which mip-federation will be copied in the Remote Host that will be the Manager of Swarm and Master of Exareme)
